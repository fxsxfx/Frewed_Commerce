...
from sql_connection import Comunicacion
...
class Ventana(Frame):
    ...
    def widgets(self):
        self.frame_uno = Frame(self.master, bg="white", height=400, width=800)
        ...
        Label(self.frame_uno, text= 'Edad', bg='white', fg='black', font=('Rockwell', 13, 'bold')).grid(column=0, row=2, pady=5)
        ...
        opciones = ["Vegetales", "Carnes", "Variado", "Limpieza"]
        combo_unidadMed = ttk.Combobox(self.frame_uno, values=opciones, font=('Comic Sans MS', 12))
        combo_unidadMed.set(opciones[0])
        combo_unidadMed.grid(row=5, column=1)
        #combobox_frame1 = ttk.Combobox(self.frame_uno, values=opciones, font=('Comic Sans MS', 12))
        # style = ttk.Style()
        # style.configure('TCombobox', foreground='blue', font=('Comic Sans MS', 12))

        # # Aplicar el estilo al Combobox
        # combobox_frame1.config(style='TCombobox')
        # combobox_frame1.set(opciones[0])
        # combobox_frame1.grid(row=5, column=1)

        Button(self.frame_uno, text='AÑADIR DATOS', font= ('Arial', 9, 'bold'), bg= '#EEAB5A', width=20, bd=3, command=self.agregar_datos).grid(column=2, row=2, pady=5, padx=5)
        Button(self.frame_uno, text='LIMPIAR CAMPOS', font= ('Arial', 9, 'bold'), bg= '#EEAB5A', width=20, bd=3, command=self.limpiar_campos).grid(column=2, row=3, pady=5, padx=5)
        Button(self.frame_uno, text='ACTUALIZAR DATOS', font= ('Arial', 9, 'bold'), bg= '#EEAB5A', width=20, bd=3, command=self.actualizar_datos).grid(column=2, row=4, pady=5, padx=5)
        Button(self.frame_uno, text='EXPORTAR A EXCEL', font= ('Arial', 9, 'bold'), bg= '#EEAB5A', width=20, bd=3, command=self.guardar_datos).grid(column=2, row=5, pady=5, padx=5)
